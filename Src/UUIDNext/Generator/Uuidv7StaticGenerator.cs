...
{
    internal static class Uuidv7StaticGenerator
    {
        private const int _sequenceMaxValue = (1 << 12) - 1;

        private static long _lastUsedTimestamp = 0;
        private static long _timestampOffset = 0;
        private static ushort _monotonicSequence = 0;

        private static readonly object _lock = new();

        internal static Guid New() => New(DateTime.UtcNow);

        private static Guid New(DateTime date)
        {
        ...
        private static void SetSequence(Span<byte> bytes, ref long timestamp)
        {
            ushort sequence;
        ...
        private static ushort GetSequenceNumber(ref long timestamp)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void EnsureTimestampNeverMoveBackward(ref long timestamp)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static Guid CreateGuidFromBigEndianBytes(Span<byte> bigEndianBytes)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void SetVersion(Span<byte> bigEndianBytes)
        {
